...
$Website_Prefix = "MHN";
$TXTFile_Extension;
include( "Reporter_Functions.php" );
...
if ($_FILES["userfile"]["error"] > 0)
  ...
  $TXTFile_Extension = substr ( $UserFile, -3 );
  echo "Report File : " .$Target_root1.$UserFile . "<br />";
...
if (  ($_FILES["userfile"]["type"] == "text/plain") && ($_FILES["userfile"]["size"] < 1024*1024*6) )
  {
  	if ($_FILES["userfile"]["error"] > 0)
    {
    ...
    if (file_exists($Target_root1 . $_FILES["userfile"]["name"]))
      ...
      move_uploaded_file($_FILES["userfile"]["tmp_name"], $Target_root1 . $_FILES["userfile"]["name"]);
...
/***************************************************************************************************/
// Upload code for photo file 2 (imagefile)
...
if (!file_exists( $Datafile ) )
	...
	$Reference = $Website_Prefix."001";  //New database so set numbers to start
	$fp = fopen( $Datafile, 'w' );
	fwrite ( $fp, "$Reference,$DataTitle,$DataSubtitle,$DataUserFile,$DataImageTitle,$DataImageFile,$UploadDate,$LiveReport\n" );
	fclose($fp);
	}
	else
	{
	echo "<br/>Data File found, current one used<br />";
	$Reference = sprintf("%s%03d", $Website_Prefix ,CountRecords( $Datafile ) + 1);
	$fp = fopen( $Datafile, 'a' );
	fwrite ( $fp, "$Reference,$DataTitle,$DataSubtitle,$DataUserFile,$DataImageTitle,$DataImageFile,$UploadDate,$LiveReport\n" );
	fclose($fp);
...
// Now call the return page
header('location:../Zerina.htm');
die();
?>
